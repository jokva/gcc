...
/* Find all nodes including non-condition in a Boolean expression.  We need to
   know the paths through the expression so that the masking and
   instrumentation phases know when to limit searches and what subgraphs must
   be threaded through, but not counted, such as a && fn (b || c) && d.

   It is essentially the intersection of downwards paths to the post-dominator
   and upwards from the post-dominator.  Finding the dominator is slightly more
   involved than picking the first/last, particularly under optimization,
   because both incoming and outgoing paths may be split.
...
find_conditions (struct function *fn)
{
    ...
    /* Bucket the Boolean expressions so that exprs[id] -> [x1, x2, ...].  */
    hash_map<int_hash<int, -1>, vec<basic_block>> exprs;
    for (basic_block b : fnblocks) {
	const int uid = basic_condition_uid (fn, b);
	/* There may be conditions created that did not get an uid, such as
	   those implicitly created by destructors.  We could fold them into
	   the condition coverage for completeness (i.e. condition coverage
	   implies (implicit) branch coverage), but they have no natural
	   buckets and should all be single-term.  For now these are ignored,
	   and coverage is left to -fprofile-arcs.  */
	if (uid <= 0)
	    continue;
		      ...
		      array_slice<sbitmap> maps, array_slice<uint64_t> masks)
{
	...
	for (edge e : b->succs)
	{
	    /* Flush the accumulators on leaving the Boolean funtion.  The
