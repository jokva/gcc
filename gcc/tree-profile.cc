...
struct conds_ctx {
    /* Output arrays. These are references and are allocated by the caller */
    basic_block *blocks;
    int *sizes;
 ...
 *   custom operators that work for arbitrary-sized bitsets.
 *
 ...
 * C [true] masks A.
 *
 * The mask is output as a bitset stored in a gcov_type_unsigned. The bitsets
 * are output in pairs, one for each decision (the outcome of the boolean
 * expression, or which arc to take in the CFG).
 */
void
find_subexpr_masks (
...
int instrument_decisions (basic_block *blocks, int nblocks, int condno)
{
    ...
    /*
     * The the true/false target blocks are included in the nblocks set, but
