...
/* Returns the condition identifier for the basic block if set, otherwise 0.
   ...
   coverage anymore, which is unreliable under optimization either way.  */
unsigned
condition_uid (basic_block b)
{
    if (!safe_is_a<gcond *> (gsi_stmt (gsi_last_bb (b))))
	return 0;
    return b->cond_uid;
}
...
find_conditions (struct function *fn)
{
    ...
    {
	const unsigned uid = condition_uid (b);
	if (uid == 0)
	    continue;
