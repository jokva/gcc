...
/* Get a fresh identifier for a new condition expression.  */
static unsigned
next_cond_uid ()
{
    return nextuid++;
}
/* Reset the condition uid, which it should be called when entering functions.
   0 is already the default/untouched value, so start at non-zero.  A valid and
   set id should always be > 0.  */
static void
reset_cond_uid ()
{
    nextuid = 1;
...
gimplify_cond_expr (tree *expr_p, gimple_seq *pre_p, fallback_t fallback)
{
  ...
  gimple_set_uid (cond_stmt, cond_uid);
